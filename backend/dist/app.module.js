"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AppModule = void 0;
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const core_1 = require("@nestjs/core");
const auth_module_1 = require("./auth/auth.module");
const database_module_1 = require("./database/database.module");
const meals_module_1 = require("./meals/meals.module");
const meal_planner_module_1 = require("./meal-planner/meal-planner.module");
const groceries_module_1 = require("./groceries/groceries.module");
const ai_module_1 = require("./ai/ai.module");
const storage_module_1 = require("./storage/storage.module");
const jwt_auth_guard_1 = require("./common/guards/jwt-auth.guard");
const rate_limit_middleware_1 = require("./common/rate-limit/rate-limit.middleware");
const request_logger_middleware_1 = require("./common/logger/request-logger.middleware");
let AppModule = class AppModule {
    configure(consumer) {
        consumer.apply(request_logger_middleware_1.RequestLoggerMiddleware, rate_limit_middleware_1.RateLimitMiddleware).forRoutes('*');
    }
};
exports.AppModule = AppModule;
exports.AppModule = AppModule = __decorate([
    (0, common_1.Module)({
        imports: [
            config_1.ConfigModule.forRoot({ isGlobal: true }),
            database_module_1.DatabaseModule,
            auth_module_1.AuthModule,
            ai_module_1.AiModule,
            storage_module_1.StorageModule,
            meals_module_1.MealsModule,
            meal_planner_module_1.MealPlannerModule,
            groceries_module_1.GroceriesModule,
        ],
        providers: [{ provide: core_1.APP_GUARD, useClass: jwt_auth_guard_1.JwtAuthGuard }],
    })
], AppModule);
//# sourceMappingURL=app.module.js.map